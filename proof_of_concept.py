...
pivot_df = pivot_df.rename_axis(None,axis=1) #移除index名稱

#計算餘弦相似度
cosine_similarities = []
length_vector_a = pow((vector_a**2).sum(),0.5)
for row in pivot_df.iterrows():
    vector_bi = np.array([row[1][1],row[1][2],row[1][3]]) #row[1]取出series
    vector_a_dot_vector_bi = np.dot(vector_a,vector_bi) #分子
...
column_names_to_revise = {
    ...
    1:"candidates_1",2:"candidates_2",3:"candidates_3"
}
